...
import { BouhekiConfigBuilder, DefaultAllowedDomains, BouhekiConfigInterface } from "../src/bouheki";
import { loadYamlFile } from "./helper";

test('BouhekiConfig initialize', () => {
    const config = new BouhekiConfigBuilder();
    expect(config.bouhekiConfig.network.domain.allow).toEqual(DefaultAllowedDomains)
})

test('allowedAddresses', () => {
    const builder = new BouhekiConfigBuilder().allowedAddresses("127.0.0.1/32,10.0.1.0/24,example.com");
    ...
    expect(builder.bouhekiConfig.network.cidr.allow).toEqual(["127.0.0.1/32", "10.0.1.0/24"])
})
